...
def _fmt_rub(x):
    if x is None:
        return "н/д"
    # неразрывный пробел в разрядах — чтобы «10 898» не ломалось по строкам
...
def _market_block(title, a):
    ...
    pos = a["pos"]
    if pos <= 33:
        sig = ("🟢 разворот у низа — можно заходить" if mom7 > 0
               else "🟡 низко, но ещё падает — подождать")
    elif pos <= 66:
        sig = "🟡 середина — ждать ближе к низу"
    else:
        sig = ("🔴 дорого и вниз — продавать" if mom7 < 0
               else "🔴 дорого — входить осторожно")
    return (f"<b>{title} — {_fmt_rub(a['cur'])} ₽/г</b>\n"
            f"Позиция: {pos}% от низа ({_fmt_rub(a['lo'])}–{_fmt_rub(a['hi'])})\n"
            f"Изм: 7д {f(7)} · 30д {f(30)} · 90д {f(90)} · 180д {f(180)}\n"
            f"Тренд 90д: {trend}\n"
            f"📥 Закуп ниже <b>{_fmt_rub(entry_zone)}</b>\n"
            f"📤 Продажа выше <b>{_fmt_rub(exit_zone)}</b>\n"
            f"Сигнал: {sig}")
