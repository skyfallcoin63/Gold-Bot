...
def _market_block(title, a):
    ...
    if a["pos"] <= 33:
        now = "🟢 сейчас в зоне закупа"
    elif a["pos"] <= 66:
        now = "🟡 сейчас в середине — ждать ближе к закупу"
    else:
        now = "🔴 сейчас дорого — ближе к зоне выхода"
    return (f"<b>{title}</b>\n"
            f"Сейчас: <b>{_fmt_rub(a['cur'])} ₽/г</b> ({a['pos']}% от низа диапазона)\n"
